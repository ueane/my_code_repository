/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 五  8/18 11:01:39 2017
 ************************************************************************/
...
void __merge_sort(int a[], int l, int r) {
...
void init_merge_sort_args(mergeSortArgs *qsa, int *arr, int l, int r, int status) {
    qsa->arr = arr;
    qsa->l = l;
    qsa->r = r;
    qsa->status = 0;
...
void merge_sort(int *arr, int n) {
    ...
    push_stack(s, qsa);
    while (!empty_stack(s)) {
        mergeSortArgs *tqsa = &top_stack(s, mergeSortArgs);
        switch (tqsa->status) {
            case 0:
                if (tqsa->r <= tqsa->l) {
                    tqsa->status = 4;
                }
                else if(tqsa->r - tqsa->l == 1) {
                    if(tqsa->arr[tqsa->l] > tqsa->arr[tqsa->r]) {
                        swap(tqsa->arr[tqsa->l], tqsa->arr[tqsa->r]);
                    }  
                    tqsa->status = 4;
                } else  {
                    tqsa->status = 2;
                ...
                break;
            case 1:
                __merge_sort(tqsa->arr, tqsa->l, tqsa->r);
                tqsa->status = 4;
                break;
            case 2:
                init_merge_sort_args(&qsa, arr, tqsa->l, (tqsa->l + tqsa->r) / 2, 0);
                push_stack(s, qsa);
                tqsa->status = 3;
                break;
            case 3:
                init_merge_sort_args(&qsa, arr, (tqsa->l + tqsa->r) / 2 + 1, tqsa->r, 0);
                push_stack(s, qsa);
                tqsa->status = 1;
                break;
            case 4:
